...
(function () {
	...
	CKEDITOR.plugins.add('mybbmycode', {
		requires: 'entities',
				...
				elements: {
					blockquote: function (element) {
						...
						if (citeText) {
							...
							while(citeValue && citeValue.substr(-1) == ' ') {//rtrim
								citeValue = citeValue.substr(0, citeValue.length-1);
							}

							if((citeValue.substr(0, 1) == "'" || citeValue.substr(0, 1) == "'" || citeValue.substr(0, 1) == "&quote;") && citeValue.substr(-1) == citeValue.substr(0, 1)) {
								citeValue = citeValue.substr(1, citeValue.length-2);
							}
							if(citeValue) {
								cite.add(new CKEDITOR.htmlParser.text(citeValue));
					...
					$: function (element) {
						...
						if (tagName in convertMap)
							...
							if (( value = style.color )) {
								...
								if (percentValue) {
									value = percentValue[1] + 'px';
									tagName = 'size';
								}
								else {
									if (['xx-small', 'x-small', 'small', 'medium', 'large', 'x-large', 'xx-large'].indexOf(value) > -1) {
										tagName = 'size';
									}
								}
							} else if (( value = style['font-family'] )) {
								var percentValue = value.split(',');
								if (percentValue) {
							...
							if (value = style['text-align']) {
								if (value != 'left' && value != 'right' && value != 'justify' && value != 'center') {
									value = 'left';
								}
								tagName = 'align';
							}
							else if (value = attributes['align']) {
								if (value != 'left' && value != 'right' && value != 'justify' && value != 'center') {
									value = 'left';
								}
								tagName = 'align';
							}
							else if (value = style['direction']) {
								if (value == 'ltr') {
									value = 'left';
									tagName = 'align';
								}
							...
							if (element.hasClass('codeblock-code')) {
								tagName = 'code';
							}
								...
								if (citeText) {
									...
									for(var child = 1; child < element.children.length; child++) {
										if(typeof element.children[child].getOuterHtml == 'function') {
											...
											quoted += element.children[child].value;
										}
									}
									element.setHtml(quoted);
								}
